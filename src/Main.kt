...
object Main {
    ...
    var heroes: Array<String> = arrayOf("Swordsman", "Wizard", "Archer", "Medic")
    ...
    fun setBossImmunityType() {
        val random = Random()
        val randomHeroIndex = random.nextInt(heroes.size - 1) // 0, 1, 2
        ...
        for (i in 0 until heroes.size - 1) {
            ...
            if (heroesHealth[i] > 0 && bossHealth > 0) {
                ...
                if (bossImmunityType !== heroes[i]) {
                    ...
                    val coeff = random.nextInt(4) + 2 // 2, 3, 4, 5
                    if (criticalChance > 3) {
                        val criticalDamage = heroesDamage[i] * coeff
